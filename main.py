...
def check_normality_big_nums(content, S, mean, criteries):
    ...
    for i in small_arrays:
        ...
        xi = ((len(i) - ni) ** 2) / ni
        table_content.append([small_arrays.index(i), midpoints[small_arrays.index(i)], len(i), xminx, phi_arg, ni, xi])
    sum_xi = 0
    for i in table_content:
        sum_xi += i[-1]
    headers = ["Номер интервала i", 'Середина интервала Xio', 'Число результатов измерений в интервале', 'Xio-Xср',
               'Zi=Xio-Xср/S', 'ni', 'xi2']
    print_table(headers, table_content)
    print(f"Суммарный xi2 выборки равен: {sum_xi}")
    print(f"Q-процентная точка для {r_def} интервалов, т.е. {f} степеней свободы, равна {criteries[f]}")
    if sum_xi >= criteries[f]:
        print(f'{sum_xi} >= {criteries[f]}, распределение похоже на нормальное по ГОСТ')
        return True
    else:
        print(f'{sum_xi} <= {criteries[f]}, распределение не похоже на нормальное по ГОСТ')
        return False
...
def others_check(sample_data, distributions_d, cexcess, mean):
    ...
    params_tri = (
    np.min(sample_data), np.mean(sample_data), np.max(sample_data))  # Используем минимум, среднее и максимум из выборки
    ...
    params_laplace = (
    np.mean(sample_data), np.std(sample_data))  # Используем среднее и стандартное отклонение из выборки
